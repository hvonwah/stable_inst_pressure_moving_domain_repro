...
parser.add_argument('-stab', '--stabil', type=str, default='sufficient', choices=['sufficient','global', 'patch'])
...
output_file = f'results/Example1_TH{k}hogeo{int(ho_geom)}subdiv{subdivlvl}'
output_file += f'hmax{h_max}BDF2dt{dt}nu{nu}gpv{gamma_v}gpp{gamma_p}'
output_file += f'gp{stabil}graddiv{gamma_gd}'
...
with open(f'{output_file}.txt', 'w') as fid:
    fid.write('time\tdrag\tlift\tdragP1cor\tdragP2cor\tdragP1\tdragP2\telsnew\telsnewext\tdiv\tmeanP\tdiffPcir\tdiffA\n')
...
def CompOutput():
    ...
    pre_diff = Integrate((pre - pre_mean) * ds, mesh)
    if len(area) > 1:
    ...
    out += f'\t{pre_mean:.5e}\t{pre_diff:.5e}\t{diff_area:.5e}\n'
...
if vtk_out:
    ...
    if subdivlvl == 0:
        coefs_v = [vel.Compile(), lsetp1.Compile(), deform.Compile(),
                   div(vel).Compile(), (Grad(vel)[1, 0] - Grad(vel)[0, 1]).Compile()]
        names_v = ['vel', 'lsetp1', 'deform', 'div', 'vort']
        coefs_p = [pre.Compile(), (pre - gamma_gd * div(vel)).Compile(),
                   lsetp1.Compile(), deform.Compile()]
        names_p = ['pre', 'pre_corr', 'lsetp1', 'deform']
    elif subdivlvl > 0:
        coefs_v = [vel.Compile(), lsetp1.Compile(), levelset.Compile(),
                   div(vel).Compile(), (Grad(vel)[1, 0] - Grad(vel)[0, 1]).Compile()]
        names_v = ['vel', 'lsetp1', 'lset', 'div', 'vort']
        coefs_p = [pre.Compile(), (pre - gamma_gd * div(vel)).Compile(),
                   lsetp1.Compile(), levelset.Compile()]
        names_p = ['pre', 'pre_corr', 'lsetp1', 'lset']

    vtk_v = VTKOutput(ma=mesh, coefs=coefs_v, names=names_v, subdivision=1,
                      filename=f'{vtk_dir_abs}/{output_file}_vel')
    vtk_p = VTKOutput(ma=mesh, coefs=coefs_p, names=names_p, subdivision=1,
                      filename=f'{vtk_dir_abs}/{output_file}_pre')
...
with TaskManager():
    ...
    if vtk_out:
        if stabil == 'global':
            ...
            vtk_v.Do(time=t.Get(), drawelems=els_outer)
            vtk_p.Do(time=t.Get(), drawelems=els_hasneg)
    Redraw()
