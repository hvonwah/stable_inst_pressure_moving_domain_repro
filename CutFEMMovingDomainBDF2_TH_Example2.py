...
output_file = f'results/Example2_TH{k}hmax{h_max}BDF2dt{dt}nu{nu}'
output_file += f'gpv{gamma_v}gpp{gamma_p}gp{stabil}graddiv{gamma_gd}'
...
a_stress = RestrictedBilinearForm(X, element_restriction=els_if,
                                  ...
                                  check_unused=False)
for bnd, n in normals.items():
    stress = InnerProduct(-nu * Grad(u) * n + (p - gamma_gd * div(u)) * n, v)
...
a_pre_cor = RestrictedBilinearForm(X, element_restriction=els_if,
                                   ...
                                   check_unused=False)
for bnd, n in normals.items():
    pre_cor = InnerProduct((p - gamma_gd * div(u)) * n, v)
...
if vtk_out:
    ...
    vtk_v = VTKOutput(ma=mesh,
                      coefs=[vel.Compile(), div(vel).Compile(),
                             (Grad(vel)[1, 0] - Grad(vel)[0, 1]).Compile(), *lsetsp1],
                      names=['vel', 'div', 'vort', 'lset1', 'lset2'],
                      filename=f'{vtk_dir_abs}/{output_file}_vel', subdivision=1)

    vtk_p = VTKOutput(ma=mesh,
                      coefs=[pre.Compile(), (pre - gamma_gd * div(vel)).Compile(), *lsetsp1],
                      names=['pre', 'pre_cor', 'lset1', 'lset2'],
                      filename=f'{vtk_dir_abs}/{output_file}_pre', subdivision=1)
...
def update_lset_and_markers(t):
    ...
    # mlci class also updates els_hasneg, els_if, els_if_single
